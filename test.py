...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        for gt, pred, pred_max_prob in zip(labels, preds_str, preds_max_prob):
            ...
            if (opt.sensitive and opt.data_filtering_off) or opt.alphanumeric:
                pred = pred.lower()
                gt = gt.lower()
...
def load_benchmark_all_eval(opt, \
    ...
    for eval_data in eval_data_list:
        ...
        evaluation_loader = torch.utils.data.DataLoader(
            ...
            num_workers=int(opt.workers),
            collate_fn=AlignCollate_evaluation, pin_memory=True)
        data_loaders[data_name] = evaluation_loader
        log_info += eval_data_log+'\n'
    return data_loaders, log_info


def train_benchmark_all_eval(model, criterion, converter, opt, iter, data_loaders, \
    eval_data_list=['IIIT5k_3000', 'SVT', 'IC03_867', 
                      'IC13_1015',  'IC15_2077', 'SVTP', 'CUTE80']):
    """ evaluation with 10 benchmark evaluation datasets """
    # The evaluation datasets, dataset order is same with Table 1 in our paper.
    ...
    total_evaluation_data_number = 0
    total_correct_number = 0
    log = open(f'./saved_models/{opt.experiment_name}/log_train_all_evaluation.txt', 'a')
    dashed_line = '-' * 80
    print(dashed_line)
    log.write(dashed_line + '\n')
    for eval_data in eval_data_list:
        evaluation_loader = data_loaders[eval_data]

        _, accuracy_by_best_model, norm_ED_by_best_model, _, _, _, infer_time, length_of_data = validation(
            model, criterion, evaluation_loader, converter, opt)
        list_accuracy.append(f'{accuracy_by_best_model:0.3f}')
        dict_accuracy[eval_data] = accuracy_by_best_model
        total_evaluation_data_number += length_of_data
        total_correct_number += accuracy_by_best_model * length_of_data

    total_accuracy = total_correct_number / total_evaluation_data_number
    evaluation_log = 'iter={}: '.format(iter)
    for name, accuracy in zip(eval_data_list, list_accuracy):
        evaluation_log += f'{name}: {accuracy}\t'
    evaluation_log += f'tot: {total_accuracy:0.3f}\t'
    print(evaluation_log)
    log.write(evaluation_log + '\n')
    log.close()
    return evaluation_log, total_accuracy, dict_accuracy
